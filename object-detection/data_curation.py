...
def parse_voc_annotation(xml_file: str) -> list[dict[str, any]] | None:
    try:
        # If the XML file cannot be parsed or opened, assume it is corrupted
        tree = ET.parse(xml_file)
        # If the annotation element is missing, assume corruption
...
def validate_voc_annotation(image_width: int, image_height: int, bounding_boxes: list[dict[str,any]]) -> list[dict[str,float]] | None:
...
def validate_coco_annotation_preindexed(image_width: int, 
                                        image_height: int, 
                                        image_id: int, 
                                        annotations_by_image_id: dict) -> list[dict[str,any]] | None:
    ...
    try:
        for line in fileinput.input(yolo_annotation_filename, inplace=True, backup='.bak'):
            ...
            if not ((0.0 <= center_x <= 1.0) and (0.0 <= center_y <= 1.0) and
                    (0.0 <= bounding_box_width <= 1.0) and (0.0 <= bounding_box_height <= 1.0)):
                continue
            if ((center_x - (bounding_box_width / 2)) < 0 or
                (center_y - (bounding_box_height / 2)) < 0 or
                (center_x + (bounding_box_width / 2)) > 1 or
                (center_y + (bounding_box_height / 2)) > 1):
                continue
    ...
    for filename in tqdm(image_files, desc="Processing images and their annotations"):
        ...
        match annotation_type.lower():
            case 'coco':
                ...
                yolo_bounding_boxes = validate_coco_annotation_preindexed(width, height, image_id, annotations_by_image_id)
...
def main() -> None:
    ...
    parser.add_argument("label_directory", type=str, nargs="?", help="Path to the labels directory. For COCO datasets, this is a path to a JSON file.")
