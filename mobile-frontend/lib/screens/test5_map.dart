...
class _LocationScreenState extends State<LocationScreen> {
  ...
  Future<void> _checkLocationPermissions() async {
    ...
    if (permission == LocationPermission.deniedForever) {
      _showAlert(
          "Permission Denied", "Enable location access in device settings.");
      return;
    }
    ...
    setState(() {
      ...
      _markers.add(
        Marker(
          ...
          infoWindow:
              InfoWindow(title: "Destination: ${locationName.toUpperCase()}"),
  ...
  Future<bool> _searchViaServer(String query) async {
    try {
      ...
      final response = await http
          .get(
            Uri.parse(
                'http://10.0.2.2:5003/geocode?query=${Uri.encodeComponent(query)}'),
          )
      ...
      if (response.statusCode == 200) {
        ...
        if (data['found'] == true) {
          _setDestination(data['name'] ?? query,
  ...
  // Fixed version of _startJourney method
  void _startJourney() {
    ...
    setState(() {
      // Toggle journey state - allows restart if already started
      _isJourneyStarted = true;

      // Clear existing routes to prepare for new journey
      _routes.clear();
    });
    ...
    String destinationName = _destinationController.text.isNotEmpty
        ? _destinationController.text
        : "destination";

    _showAlert("Journey Started",
        "Navigation to $destinationName has begun. Stay alert for elephants nearby!");

    // Start checking for elephants immediately
    _checkForElephantsNearby();
  }
    ...
    return Scaffold(
      body: Stack(
        children: [
          ...
          // Add a status indicator for journey
          if (_isJourneyStarted)
            Positioned(
              bottom: 140,
              left: 10,
        ...
        children: [
          ...
          FloatingActionButton.extended(
            ...
            onPressed: (_destination != null && _currentLocation != null)
                ? _startJourney
                : null,
            label: Text(_isJourneyStarted ? "Restart Journey" : "Start Journey",
